...
export default class PdfProductsTemplate {
  private static generateTableRow(doc, product, variant, sum) {
    const startY = doc.y;
    doc
      .fontSize(10)
  ...
  static generateTable(
    ...
    this.generateTableRow(doc, "Producto", "Variante", "Suma");
    moveDown(doc);
    generateHr(doc);
    moveDown(doc);
    doc.font("Helvetica");
  ...
  static generateHeader(doc): void {
    ...
    doc.text("Productos destacados", 70, doc.y, { align: "left" });
