...
function transformToDiscountsTopTable(
  result: DiscountsCountPopularityResult
): DiscountsTopTableRow[] {
  const currentMap = new Map<string, DiscountsTopTableRow>();

  result.current.forEach((currentItem) => {
    const currentCount = currentMap.get(currentItem.discountId)
      ? currentMap.get(currentItem.discountId).sum
      : "0";
...
const DiscountsTopByCount = ({
  ...
  if (data.analytics == undefined) {
    return <Heading level="h3">No puedo obtener pedidos ni descuentos</Heading>;
  }

  if (data.analytics.dateRangeFrom) {
    return (
      <DiscountsTopTable
        tableRows={transformToDiscountsTopTable(data.analytics)}
      />
    );
  } else {
    return (
      <Heading level="h3">No hay descuentos para pedidos seleccionados</Heading>
    );
  }
};

export const DiscountsTopCard = ({
  orderStatuses,
  dateRange,
  dateRangeCompareTo,
}: {
    ...
    <Grid container paddingBottom={2} spacing={3}>
      <Grid item xs={12} md={12}>
        <Grid container spacing={2}>
          ...
          <Grid item>
            <Heading level="h2">Grandes descuentos</Heading>
          </Grid>
        </Grid>
      </Grid>
      <Grid item xs={12} md={12}>
