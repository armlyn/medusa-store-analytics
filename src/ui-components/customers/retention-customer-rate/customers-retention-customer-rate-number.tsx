...
export const RetentionCustomerRateNumber = ({
  retentionCustomerRateResponse,
  compareEnabled,
}: {
  retentionCustomerRateResponse: CustomersRetentionCustomerRateResponse;
  compareEnabled?: boolean;
}) => {
  const currentPercentage: number | undefined =
    retentionCustomerRateResponse.analytics.current !== undefined
      ? parseInt(retentionCustomerRateResponse.analytics.current)
      : undefined;
  const previousPercentage: number | undefined =
    retentionCustomerRateResponse.analytics.previous !== undefined
      ? parseInt(retentionCustomerRateResponse.analytics.previous)
      : undefined;
  ...
  return (
    <Grid container alignItems={"center"} spacing={2}>
      ...
      {compareEnabled &&
        retentionCustomerRateResponse.analytics.dateRangeFromCompareTo &&
        currentPercentage !== undefined && (
          <Grid item>
