...
import {
  ReturnedVariantsTable,
  VariantsTopTableRow,
} from "./returned-variants-table";
...
function transformToVariantTopTable(
  result: VariantsCountPopularityResult
): VariantsTopTableRow[] {
  const currentMap = new Map<string, VariantsTopTableRow>();

  result.current.forEach((currentItem) => {
    const currentCount = currentMap.get(currentItem.variantId)
      ? currentMap.get(currentItem.variantId).sum
      : "0";
...
const ReturnedVariantsByCount = ({
  ...
  >(`/products-analytics/returned-by-count`, [dateRange, dateRangeCompareTo], {
  ...
  if (data.analytics.dateRangeFrom) {
    ...
    return (
      <Heading level="h3">
        No hay productos para los pedidos seleccionados
      </Heading>
    );
  }
};

export const ReturnedVariantsByCountCard = ({
  dateRange,
  dateRangeCompareTo,
}: {
  dateRange?: DateRange;
    ...
    <Grid container paddingBottom={2} spacing={3}>
      <Grid item xs={12} md={12}>
        <Grid container spacing={2} alignItems={"center"}>
          ...
          <Grid item>
            <Heading level="h2">Principales variantes devueltas</Heading>
          </Grid>
        </Grid>
      </Grid>
      <Grid item xs={12} md={12}>
        <ReturnedVariantsByCount
          dateRange={dateRange}
